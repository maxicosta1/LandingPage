...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (contactForm) {
    ...
    const validateContactForm = () => {
      ...
      if (getTrimmedValue("nombre").length < 2) {
      ...
      if (getTrimmedValue("telefono").length < 6) {
        return "Ingresá un teléfono válido.";
      }

      if (getTrimmedValue("mensaje").length < 10) {
        return "Contanos un poco más sobre tu proyecto.";
      }
    ...
    contactForm.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        contactForm.reset();
        setStatus(result.message || "Consulta enviada correctamente.", "success");
      } catch (error) {
        console.error("[contact-form] Submit failed:", error);
        setStatus(error.message || "No pudimos enviar la consulta. Intentá nuevamente.", "error");
      } finally {
